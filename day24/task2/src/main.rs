...
impl World {
    ...
    fn solve(&mut self, starting_steps: usize, direction: bool) -> usize {
        self.discover_states();
        let _start = (-1, 0);
        let _end = (
            self.world_states[0].len() as i32,
            self.world_states[0][0].len() as i32 - 1,
        );

        let step_diff = if direction {
            [(1, 0), (0, 1), (0, 0), (-1, 0), (0, -1)]
        } else {
            [(-1, 0), (0, -1), (0, 0), (1, 0), (0, 1)]
        };
        ...
        stack.push((
            start,
            starting_steps,
            starting_steps % self.world_states.len(),
        ));
...
fn main() -> io::Result<()> {
    ...
    let solution1 = w.solve(0, false);
    println!("solution1: {}", solution1);
    let solution2 = w.solve(solution1, true);
    println!("solution2: {}", solution2);
    let solution3 = w.solve(solution2 + solution1, false);
    println!("solution3: {}", solution3);
    println!("{}", solution1 + solution2 + solution3);
