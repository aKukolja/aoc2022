...
impl State {
    ...
    fn mutations(&self, f: &Factory) -> impl Iterator<Item=Self> {
        let mut some_ore = None;
        let mut some_clay = None;
        let mut some_obsidian = None;
        let mut some_geode = None;
        if self.robots.2 < f.max_ore {  // should create ore robot, else have enough
            some_ore = self.wait_and_create(&f.ore_cost, &(0, 0, 1));
        }
        if self.robots.1 < f.obsidian_cost.1 {  // should create clay robot
            some_clay = self.wait_and_create(&f.clay_cost, &(0, 1, 0));
        }
        if self.robots.0 < f.geode_cost.0 && self.robots.1 > 0{  // should create obsidian robot, avoid following state which never reaches the end
            some_obsidian = self.wait_and_create(&f.obsidian_cost, &(1, 0, 0));
        }
        if self.robots.0 > 0 {
            for (remaining, elapsed) in (1..self.time_remaining).rev().zip(0..) {
                let current_resources = resource_add(self.collected_resources, resource_mul(self.robots, elapsed));
                if let Some(remaining_resources) = resource_sub(current_resources, f.geode_cost) {
                    some_geode = Some(State{
                        time_remaining: remaining,
