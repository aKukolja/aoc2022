...
impl Mountains {
    ...
    fn add_row(&mut self, row: Vec<Element>) {
        ...
        for (width_index, e) in row.iter().enumerate() {
            match e {
                Element::Start => {
                    self.starts.insert((height_index, width_index));
                }
                Element::End => {
    ...
    fn bfs_all(&self) -> usize {
        self.starts.iter().map(|si| self.bfs(si)).min().unwrap()
    }

    fn bfs(&self, si: &Coord) -> usize {
                ...
                _ => {
                    ...
                    for direction in directions {
                        ...
                        match next_element {
                            ...
                            Element::End => {
                                if TERMINATOR - curr_height <= 1 {
                                    q.push_back(((next_height, next_width), path_len + 1));
                                }
                            }
                            Element::Value(vvv) => {
                                if vvv - curr_height <= 1 {
                                    q.push_back(((next_height, next_width), path_len + 1));
                                }
                            }
                        }
