...
fn iter_tree(root: &DirHandle) -> u64 {
    let mut curr_sum = root.borrow().size;  // start with sum of files
    for (_, child) in &root.borrow_mut().children {
        let child_sum = iter_tree(child);
        curr_sum += child_sum;  // add child directory to our size
    }
    root.borrow_mut().size = curr_sum;
    return curr_sum;
}
fn find_candidate(root: &DirHandle, target: u64) -> Option<u64> {
    let mut candidate = root.borrow().size;
    if candidate < target {
        return None;  // if this direcotry size is too small all childern will be smaller
    }
    for (_, child) in &root.borrow_mut().children {
        match find_candidate(child, target) {
            Some(val) => {
                if val >= target {
                    candidate = min(candidate, val);
                }
            }
            None => {}
        }
...
fn main() {
    ...
    let _ = iter_tree(&root);
    let target = MIN_FREE as i64 - (TOTAL_SPACE as i64 - root.borrow().size as i64);
    let removed = find_candidate(&root, target as u64).unwrap();
    println!("{}", removed);
} 
